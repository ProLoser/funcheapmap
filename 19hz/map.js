 ...
 */
window.updateSpotifyPlayer = function(artistName, element) {
  const iframe = document.getElementById('spotify-iframe');
  if (iframe) {
    const encodedArtist = encodeURIComponent(artistName);
    const escapedArtist = Events.escapeHtml(artistName);
    iframe.src = `https://open.spotify.com/embed/search/${encodedArtist}?utm_source=generator`;
    iframe.title = `Spotify player for ${escapedArtist}`;
    
    // Update active state
    const container = element.closest('.artist-selector');
    if (container) {
          ...
          artistSelector = `
            <div class="artist-selector">
              <p><strong>Artists:</strong> ${event.artists.map((artist, index) => {
                ...
                return `<button type="button" class="artist-item${isActive}" data-artist="${artist}" aria-label="Play music by ${escapedArtist}">
                  <svg class="speaker-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">
      ...
      if (artistSelector) {
        artistSelector.addEventListener('click', (e) => {
          ...
          if (artistButton) {
            ...
            window.updateSpotifyPlayer(artistName, artistButton);
          }
        });
      }
